            ...
            <div className="max-w-xl space-y-8">
              ...
              <form className="space-y-6" onSubmit={event => { event.preventDefault(); void save(); }}>
                <fieldset disabled={!!busy} className="space-y-6 disabled:opacity-60">
                  ...
                  <label className="block space-y-2"><span className="font-mono text-xs uppercase text-muted-foreground">API Key</span><Input aria-label="API Key" type="password" autoComplete="off" value={form.apiKey} onChange={event => { setForm({ ...form, apiKey: event.target.value }); setStatus(''); }} placeholder="输入 API Key" className="font-mono" /><span className="flex items-start gap-2 text-xs leading-5 text-muted-foreground"><KeyRound className="mt-0.5 size-3.5 shrink-0" />{isDesktop() ? '保存在系统凭据库，不写入项目或任务记录。' : '仅保存在本次浏览器中；关闭标签页后需重新填写。'}</span></label>
